      ...
      <div className="mx-auto max-w-6xl">
        <div className="mx-auto max-w-xl text-center">
          <h2 className="font-heading text-3xl font-semibold tracking-tight text-foreground sm:text-4xl">
            Comece a agendar em poucos minutos
          </h2>
          <p className="mt-3 text-muted-foreground">
            É simples, rápido e sem complicação. Em 3 passos você já está recebendo agendamentos.
          </p>
        </div>
          ...
          {PASSOS.map((passo, index) => (
            <div key={passo.numero} className="relative flex flex-col items-center text-center">
              {index < PASSOS.length - 1 ? (
                <div
                  className="absolute top-8 left-[calc(50%+2.5rem)] hidden h-px w-[calc(100%-5rem)] border-t-2 border-dashed border-border sm:block"
                  aria-hidden="true"
                />
              ) : null}
