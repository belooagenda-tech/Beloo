-- Beloo — "dar baixa" num agendamento: grava o pagamento (nunca duplica,
-- sempre atualiza o mesmo registro), desconta 1 crédito do plano quando
-- aplicável, e marca o agendamento como concluído. Tudo em uma transação.
...
revoke all on function public.complete_appointment_payment(uuid, numeric, text, text) from public;
grant execute on function public.complete_appointment_payment(uuid, numeric, text, text) to authenticated;
